...
# field_ids in the unpaid-balance form whose answer is a yes/no boolean.
UNPAID_BOOL_FIELDS = {
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Ads Cancel - Unacceptable Business - @huyit32")
    ...
    def _build_ui(self):
        ...
        self.tag_secondary_edit = QLineEdit(
            ",".join(str(x) for x in auto.APPEAL_UNACCEPTABLE_TAG_IDS_SECONDARY))
        ...
        # Single 16-question form panel.
        self._build_form_panel(v, "Unacceptable business practices answers (16Q)",
        ...
        self.btn_submit_unpaid = QPushButton("Submit unpaid-balance appeal")
        self.btn_submit_unpaid.setStyleSheet("background-color:#1a73e8; color:white; padding:8px 16px;")
        self.btn_submit_unpaid.setToolTip(
            "Unpaid-balance / future-payment-concerns questionnaire\n"
            "(17 questions, one shared set of answers applied to every checked account)."
        )
        self.btn_submit_unpaid.clicked.connect(self.submit_checked_unpaid)
        row3.addWidget(self.btn_submit_unpaid)
    ...
    def _build_form_panel(self, parent_layout, title: str, template: list) -> None:
        """Add a collapsible group box for the single appeal-form template."""
        ...
        for fid, qtext, default in template:
            ...
            self.form_widgets[fid] = w
            form.addRow(qtext, w)
        box.toggled.connect(
            lambda on: [w.setVisible(on) for w in self.form_widgets.values()]
        )
        parent_layout.addWidget(box)

    def _answers_from_ui(self) -> dict:
        out = {}
        for fid, w in self.form_widgets.items():
    ...
    def submit_checked_unpaid(self):
        """Submit the unpaid-balance 17-question form for every checked row,
        using one shared set of answers."""
        if not self.cfg:
        ...
        tag_primary   = parse_int_csv(self.tag_primary_edit.text(),
                                       auto.APPEAL_UNACCEPTABLE_TAG_IDS_PRIMARY)
        tag_secondary = parse_int_csv(self.tag_secondary_edit.text(),
                                       auto.APPEAL_UNACCEPTABLE_TAG_IDS_SECONDARY)

        delay = self.delay_spin.value()
        threads = self.threads_spin.value()
        eta = len(targets) * delay / 60 / max(1, threads)
        ans = QMessageBox.question(
            self, "Confirm — Unacceptable business practices",
            f"Submit the 16-question appeal for {len(targets)} accounts "
        ...
        self.append_log(
            f"--- Submitting {len(targets)} UNACCEPTABLE-BIZ appeals "
            f"({threads} threads) ---"
        )
        self.append_log(
            f"abuse_tag_ids primary={tag_primary} secondary={tag_secondary}"
        ...
        self.submit_worker = SubmitWorker(
            ...
            threads=threads, answers=answers,
    ...
    def _begin_submit(self):
        self.btn_submit_unpaid.setEnabled(False)
    ...
    def on_submit_done(self):
        self.btn_submit_unpaid.setEnabled(True)
        self.btn_stop.setEnabled(False)
        self.btn_scan.setEnabled(True)
